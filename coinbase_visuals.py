...
class DataManager:
    def __init__(
        self, max_snapshots=100, order_book_depth=200, volume_spike_threshold=61
    ):
        ...
        while not self.message_queue.empty():
            ...
            try:
                ...
                if data["type"] == "snapshot":
                    ...
                    self.bid_data = {
                        float(bid[0]): float(bid[1]) for bid in data["bids"]
                    }
                    self.ask_data = {
                        float(ask[0]): float(ask[1]) for ask in data["asks"]
                    }
    ...
    def record_current_snapshot(self, mid_price):
        ...
        self.historical_depth.append(
            (verts, cols)
        )  # Retain correct colors for this snapshot
...
class VaporWaveOrderBookVisualizer:
    ...
    def _init_scene(self):
        ...
        # Main wireframe
        self.batched_wireframe = scene.visuals.Line(
            parent=self.view.scene, connect="segments", width=1
        )

        # Tron bloom: ghost wireframe
        self.ghost_wireframe = scene.visuals.Line(
            parent=self.view.scene, connect="segments", width=1
        )
        self.ghost_wireframe.set_gl_state(
            blend=True, depth_test=True, blend_func=("src_alpha", "one")
        )

        # WHITE OUTLINE: Latest snapshot highlight
        self.outline_wireframe = scene.visuals.Line(
    ...
    def on_timer(self, event):
        ...
        self.batched_wireframe.set_data(
            pos=merged_verts, color=merged_cols, connect="segments"
        )
        ...
        self.ghost_wireframe.set_data(
            pos=ghost_verts, color=ghost_cols, connect="segments"
        )
        ...
        if len(self.data.historical_depth) >= 1:
            ...
            self.outline_wireframe.set_data(
                pos=outline_verts, color=outline_cols, connect="segments"
            )
    ...
    def update_x_axis_labels(self, mid_price):
        ...
        bid_prices = sorted(self.data.bid_data.keys(), reverse=True)[
        ...
        for i, label in enumerate(self.x_axis_labels):
            ...
            label.transform = scene.transforms.STTransform(
                translate=(x_position, 0, -2)
